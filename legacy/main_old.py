...
dataset = "scifact"  # Change to dataset being used
base_path = os.path.join("datasets", dataset)
doc_folder_path = os.path.join(base_path, "corpus.jsonl")
query_file_path = os.path.join(base_path, "queries.jsonl")
qrels_file_path = os.path.join(base_path, "qrels", "test.tsv")
...
documents = []
qrels = parse_qrels_from_tsv(qrels_file_path)
queries = parse_queries_from_file(query_file_path)
queries = filter_queries_by_qrels_and_odd(queries, qrels)
print("Filtered test odd queries:", len(queries))
...
if os.path.exists(preprocessed_docs_path):
    ...
    raw_documents = parse_documents_from_file(doc_folder_path)
...
results_file = "Results"   #Change to Results.txt for TREC formatting
start_time = time.time()
beir_results = {}
